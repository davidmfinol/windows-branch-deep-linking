namespace BranchSdk {
    public class BranchError {
        ...
        public const int ERR_NO_SESSION = -101;
        /* Error processing request since app doesn't have internet permission. */
        public const int ERR_NO_INTERNET_PERMISSION = -102;
        /* Error processing request since referral code provided is invalid. */
        public const int ERR_INVALID_REFERRAL_CODE = -103;
        /* Error processing request since Branch is not initialised. */
        public const int ERR_BRANCH_INIT_FAILED = -104;
        /* Error processing request since alias is already used. */
        public const int ERR_BRANCH_DUPLICATE_URL = -105;
        /* Error processing request since alias is already used. */
        public const int ERR_BRANCH_DUPLICATE_REFERRAL_CODE = -106;
        /* Error redeeming rewards. */
        public const int ERR_BRANCH_REDEEM_REWARD = -107;
        /* Error with API level below 14. */
        public const int ERR_API_LVL_14_NEEDED = -108;
        /* Error Branch is not instantiated. */
        public const int ERR_BRANCH_NOT_INSTANTIATED = -109;
        /* Error while creating share options. */
        public const int ERR_BRANCH_NO_SHARE_OPTION = -110;
        /* Request Branch server timed out. */
        public const int ERR_BRANCH_REQ_TIMED_OUT = -111;
        /* Request failed to hit branch servers */
        public const int ERR_BRANCH_UNABLE_TO_REACH_SERVERS = -112;
        /* Request failed due to poor connectivity */
        public const int ERR_BRANCH_NO_CONNECTIVITY = -113;
        /* Branch key is not specified or invalid */
        public const int ERR_BRANCH_KEY_INVALID = -114;
        /* Request failed due to resource conflict */
        public const int ERR_BRANCH_RESOURCE_CONFLICT = -115;
        /* Branch request is invalid */
        public const int ERR_BRANCH_INVALID_REQUEST = -116;
        /* Tracking is disabled. Requested operations will not work when tracking is disabled */
        public const int ERR_BRANCH_TRACKING_DISABLED = -117;
        ...
        public BranchError(string failMsg, int statusCode) {
            errorMessage = failMsg + InitErrorCodeAndGetLocalisedMessage(statusCode);
        }
